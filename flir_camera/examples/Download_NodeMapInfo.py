...
# Zhiang Chen, July 2025
Nodeinfo_string_output = ""

def parse_indented_text_to_dict(text):
    ...
    for line in lines:
        ...
        if indent % 4 != 0:
            print(f"Warning: skipping malformed line (bad indent): {line}")
            continue

        # Ensure stack always has a root level
        while stack and indent < stack[-1][0]:
            stack.pop()

        if not stack:
            # Reinitialize root if stack was completely popped
            stack = [(0, root)]
        ...
        if ':' in line:
            ...
            key = line.strip()
            current_dict[key] = {}
            stack.append((indent + 4, current_dict[key]))  # Increase indent level expected for children
...
def print_with_indent(level, text):
    ...
    for i in range(level):
        ind += '    '
    print('%s%s' % (ind, text))
    # add the output to the global variable Nodeinfo_string_output
    global Nodeinfo_string_output
    Nodeinfo_string_output += '%s%s\n' % (ind, text)
...
def main():
    ...
    for i, cam in enumerate(cam_list):
        ...
        # Sparse Nodeinfo_string_output to a dictionary
        print('x'* 80)
        print(Nodeinfo_string_output)
        print('x'* 80)
        nodeinfo_dict = parse_indented_text_to_dict(Nodeinfo_string_output)

        # Save the node information to a YAML file
        # The file will be named "NodeMapInfo_Camera_<serial_number>.yaml"
        cam_serial = cam.GetUniqueID()
        cam_serial = cam_serial.replace(' ', '_')  # Replace spaces with underscores for file name
        # Replace \ with _ for file name compatibility
        cam_serial = cam_serial.replace('\\', '_')
        # Replace & with _ for file name compatibility
        cam_serial = cam_serial.replace('&', '_')
        filename = f'data/{cam_serial}.yaml'

        with open(filename, 'w') as file:
            yaml.dump(nodeinfo_dict, file, default_flow_style=False, sort_keys=False)
