"""
Mission upload test with comprehensive error handling and proper protocol implementation
"""
...
def clear_mission(mav, target_system, target_component):
    """Clear existing mission before uploading new one"""
    print(f"\n   Clearing existing mission...")
    ...
    print("   ⚠ Timeout waiting for mission clear ACK (proceeding anyway)")
    return True  # Proceed even if we don't get ACK

def test_mission_upload():
    """Test mission upload with comprehensive error handling"""
    
    print("="*70)
    print("MISSION UPLOAD TEST WITH ERROR HANDLING")
    print("="*70)
    ...
    print(f"\n4. CLEARING EXISTING MISSION")
    clear_mission(mav, target_system, target_component)
    
    # Small delay after clearing
    time.sleep(0.5)
    
    # Send mission count
    print(f"\n5. SENDING MISSION COUNT")
    ...
    while waypoints_sent < mission_count and time.time() < timeout_time:
        ...
        if msg_type in ['MISSION_REQUEST', 'MISSION_REQUEST_INT']:
            seq = msg.seq
            print(f"   Received {msg_type} for waypoint {seq}")
            
            if seq >= len(waypoints):
                print(f"   ✗ ERROR: Requested waypoint {seq} out of range")
                continue
            
            if seq in requests_received:
                print(f"   ⚠ WARNING: Duplicate request for waypoint {seq}")
            
            requests_received.add(seq)
            wp = waypoints[seq]
            ...
            if ack_type == mavutil.mavlink.MAV_MISSION_ACCEPTED:
                ...
                if ack_type == 15:
                    print("\n   HINT: Error 15 'Not in a mission' typically means:")
                    print("   - The autopilot is not in AUTO or GUIDED mode")
                    print("   - Try switching to AUTO mode before uploading")
                    print("   - Or ignore this error - mission is usually still loaded")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("1. Vehicle not connected (no heartbeat)")
        print("2. Wrong connection string (check UDP port)")
        print("3. Firewall blocking connection")
        print("4. Mission format incompatible")
        print("5. Vehicle in wrong mode (error 15)")
    print("="*70 + "\n")
